...
/*! init the stuff for handling gallery && search */
( function( doc ) {
	"use strict"; // jshint ~_~

	/**
	 * non-jquery bind-method for events
	 */
	function bind(element, event, callback) {
		if(element.addEventListener)
			element.addEventListener(event, callback, false);
	...
	var button	= doc.querySelector( '#site-search-button a' ),
		search	= doc.getElementById( 'site-search-form' ),
		visible	= false;

	search.classList.add( 'site-search-invisible' );
	search.setAttribute( 'aria-hidden', 'true' );
	search.setAttribute( 'aria-visible', 'false' );

	// bind click event to search-button
	bind( button, 'click', function( /* Event */ e ) {
		e.preventDefault();
		visible = !visible;
		search.classList.toggle( 'site-search-invisible' );
		search.setAttribute( 'aria-hidden', visible ? 'false' : 'true' );
		search.setAttribute( 'aria-visible', visible ? 'true' : 'false' );
	} );
